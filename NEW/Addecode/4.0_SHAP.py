...
#  Remove Subject_ID and Age from SHAP matrix
feature_names = [col for col in df_shap.columns if col not in ["Subject_ID", "Age"]]
...
# SHAP brain regions in predicting brain age using node-level features (FA, MD, Volume, and clustering coefficient)
...
class NodeOnlyModel(nn.Module):
    def __init__(self, model):
        ...
        self.gnn1 = model.gnn1; self.bn1 = model.bn1
        self.gnn2 = model.gnn2; self.bn2 = model.bn2
        self.gnn3 = model.gnn3; self.bn3 = model.bn3
        self.gnn4 = model.gnn4; self.bn4 = model.bn4
        self.dropout = model.dropout
        self.fc = model.fc
        self.fake_global = torch.zeros(1, 64)

        # === FAKE edge index for SHAP
        indices = torch.triu(torch.ones(84, 84), diagonal=1).nonzero().T
        self.edge_index = indices.long()
        self.edge_attr = torch.ones(indices.shape[1], 1)  # dummy edge weights

    def forward(self, x_batch):
        outputs = []
        for x in x_batch:  # x: [84, 4]
            x = self.node_embed(x)
