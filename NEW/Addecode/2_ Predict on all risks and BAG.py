...
# ===  4: Save CSV
BAG_with_all_metadata.to_csv("BAG_with_all_metadata.csv", index=False)
print("Saved: BAG_with_all_metadata.csv")
...
# === Replace genotype codes with APOE labels
genotype_labels = {
    0: "APOE23",
    1: "APOE33",
    2: "APOE34",
...
# === Define order of genotypes
genotype_order = ["APOE23", "APOE33", "APOE34", "APOE44"]

# === Violin plot: BAG vs Genotype (labeled)
plt.figure(figsize=(8, 5))
sns.violinplot(data=df, x="genotype_label", y="BAG", order=genotype_order, inner="box", palette="pastel")
...
# === Violin plot: cBAG vs Genotype (labeled)
plt.figure(figsize=(8, 5))
sns.violinplot(data=df, x="genotype_label", y="cBAG", order=genotype_order, inner="box", palette="pastel")
...
# === Define order of APOE labels
apoe_order = ["E4-", "E4+"]

# === Violin plot: BAG vs APOE
plt.figure(figsize=(7, 5))
...
# === Filter to valid sex values (Male / Female) ===
df = df[df["sex"].isin(["M", "F"])].copy()

# === Set display order: Female → Male ===
