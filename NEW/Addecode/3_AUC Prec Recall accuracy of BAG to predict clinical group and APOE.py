...
# === Load AD-DECODE prediction file ===
df = pd.read_csv("/home/bas/Desktop/Paula/GATS/Better/BEST2/PCA genes/BAG/brain_age_predictions_with_metadata.csv")

# === Define APOE E4+ group: APOE == "E4+" → 1, else 0 ===
df["Group"] = (df["APOE"] == "E4+").astype(int)
y_true = df["Group"].values
...
# === Loop through both BAG and cBAG ===
for score_type in ["BAG", "cBAG"]:
    ...
    # === Prioritize recall: find lowest threshold with recall ≥ 0.60 ===
    recall_target = 0.60
    idx_candidates = np.where(tpr >= recall_target)[0]
    
    if len(idx_candidates) > 0:
        best_idx = idx_candidates[0]  # First threshold that satisfies recall ≥ 0.60
        best_threshold = thresholds[best_idx]
    else:
        # Fall back to best Youden’s J if recall ≥ 0.60 not possible
        j_scores = tpr - fpr
        best_idx = j_scores.argmax()
        best_threshold = thresholds[best_idx]
    ...
    # === Print results ===
    print(f"\n=== APOE Classification using {score_type} (Recall ≥ {recall_target}) ===")
    print(f"AUC: {auc_value:.3f}")
    print(f"Threshold (Recall ≥ {recall_target}): {best_threshold:.3f}")
    print(f"Accuracy: {acc:.3f}")
    print(f"Recall: {rec:.3f}")
    print(f"Precision: {prec:.3f}")
    print(f"F1-score: {f1:.3f}")
    ...
    plt.plot([0, 1], [0, 1], linestyle='--', color='gray')
    plt.title(f"ROC Curve for APOE E4+ Prediction using {score_type}")
    plt.xlabel("False Positive Rate")
    plt.ylabel("True Positive Rate")
    plt.legend()
    plt.grid(True)
    ...
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=["E4−", "E4+"])
    disp.plot(cmap='Blues')
    plt.title(f"Confusion Matrix: {score_type} (Recall ≥ {recall_target})")
    plt.tight_layout()
    plt.show()
    ...
    plt.figure(figsize=(6, 4))
    sns.violinplot(data=df, x="APOE_Label", y=score_type, palette="Set2", inner="box", order=["E4−", "E4+"])
    plt.title(f"{score_type} Distribution by APOE Status (AD-DECODE)")
    plt.tight_layout()
    plt.show()
...
# === Load the AD-DECODE prediction file ===
df = pd.read_csv("/home/bas/Desktop/Paula/GATS/Better/BEST2/PCA genes/BAG/brain_age_predictions_with_metadata.csv")

# === Filter for only the relevant groups: NoRisk, Familial, MCI, AD ===
df_risk = df[df["Risk"].isin(["NoRisk", "Familial", "MCI", "AD"])].copy()

# === Define binary classification target: 1 = MCI/AD, 0 = NoRisk/Familial ===
df_risk["Group"] = df_risk["Risk"].isin(["MCI", "AD"]).astype(int)
y_true = df_risk["Group"].values

# === Define target recall level ===
recall_target = 0.60
...
for score_type in ["BAG", "cBAG"]:
    ...
    # Select first threshold that gives recall ≥ recall_target
    idx_candidates = np.where(tpr >= recall_target)[0]
    if len(idx_candidates) > 0:
        best_idx = idx_candidates[0]
        best_threshold = thresholds[best_idx]
    else:
        # If recall target not met, fall back to Youden’s J
        j_scores = tpr - fpr
        best_idx = j_scores.argmax()
        best_threshold = thresholds[best_idx]
    ...
    # === Print results ===
    print(f"\n=== RISK Classification using {score_type} (Recall ≥ {recall_target}) ===")
    print(f"AUC: {auc_value:.3f}")
    print(f"Threshold (Recall ≥ {recall_target}): {best_threshold:.3f}")
    print(f"Accuracy: {acc:.3f}")
    print(f"Recall: {rec:.3f}")
    ...
    plt.scatter(fpr[best_idx], tpr[best_idx], color='red', label=f"Threshold = {best_threshold:.2f}")
    plt.plot([0, 1], [0, 1], linestyle='--', color='gray')
    plt.title(f"ROC Curve: NoRisk+Familial vs MCI+AD using {score_type}")
    plt.xlabel("False Positive Rate")
    plt.ylabel("True Positive Rate")
    plt.legend()
    plt.grid(True)
    ...
    df_plot["Group_Label"] = df_plot["Risk"].apply(lambda x: "NoRisk+Familial" if x in ["NoRisk", "Familial"] else "MCI+AD")
    plt.figure(figsize=(6, 4))
    sns.violinplot(data=df_plot, x="Group_Label", y=score_type, palette="Set2", inner="box", order=["NoRisk+Familial", "MCI+AD"])
    plt.title(f"{score_type} Distribution: NoRisk+Familial vs MCI+AD")
    plt.tight_layout()
    plt.show()
