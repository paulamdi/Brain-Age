...
plt.title("Predicted vs Real Ages (ADRC Bimodal Model)")
plt.legend()
...
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "ADRC_bimodal_ScatterPlot_realVSPred.png"), dpi=300)
plt.close()
...
# === SCATTER PLOT WITH TREND LINE & EQUATION IN LEGEND ===
import os
import numpy as np
import matplotlib.pyplot as plt
...
# === Scatter plot with ALL prediction points (all folds & repeats) ===
plt.figure(figsize=(8, 6))
plt.scatter(
    df_preds_bimodal["Real_Age"],
    df_preds_bimodal["Predicted_Age"],
    alpha=0.7,
    edgecolors='k',
...
reg_all = LinearRegression().fit(
    df_preds_bimodal["Real_Age"].values.reshape(-1, 1),
    df_preds_bimodal["Predicted_Age"].values
)
slope = reg_all.coef_[0]
intercept = reg_all.intercept_

# === Trend line over full plot range ===
min_age = min(df_preds_bimodal["Real_Age"].min(), df_preds_bimodal["Predicted_Age"].min()) - 5
max_age = max(df_preds_bimodal["Real_Age"].max(), df_preds_bimodal["Predicted_Age"].max()) + 5
x_line = np.array([min_age, max_age]).reshape(-1, 1)
y_line = reg_all.predict(x_line)

# === Plot linear regression line with equation in legend ===
eq_label = fr"$y = {slope:.2f}x + {intercept:.2f}$"
plt.plot(x_line, y_line,
         color='navy', linestyle='-', lw=2, alpha=0.5,
         label=eq_label)

# === Plot ideal y = x line ===
plt.plot([min_age, max_age], [min_age, max_age],
         color='crimson', linestyle='--', lw=2, label="Ideal (y = x)")

# === Metrics (mean ± std from repetitions,  already defined) ===
# mae_mean, mae_std
# rmse_mean, rmse_std
# r2_mean, r2_std
textstr = (
...
plt.xlabel("Real Age")
plt.ylabel("Predicted Age")
plt.title("Predicted vs Real Age – All Repeats (Bimodal ADRC)")
plt.legend()
plt.grid(True)
plt.tight_layout()

# === Save figure ===
plt.savefig(os.path.join(output_dir, "ADRC_BIMODAL_scatter_real_vs_pred_allpoints_with_trend.png"), dpi=300)
plt.close()
...
#SCATTER PLOT WITH THE MEAN BETWEEN REPS
# === Average predictions per subject across repetitions ===
df_means = df_preds_bimodal.groupby("Subject_ID").agg({
    "Real_Age": "first",
    "Predicted_Age": "mean"
}).reset_index()

# === Compute metrics on df_means (no std) ===
mae_mean_subj  = mean_absolute_error(df_means["Real_Age"], df_means["Predicted_Age"])
rmse_mean_subj = mean_squared_error(df_means["Real_Age"], df_means["Predicted_Age"], squared=False)
r2_mean_subj   = r2_score(df_means["Real_Age"], df_means["Predicted_Age"])




# === Scatter plot (1 point per subjet) ===
plt.figure(figsize=(8, 6))
plt.scatter(df_means["Real_Age"], df_means["Predicted_Age"],
            alpha=0.7, edgecolors='k', label="Mean Prediction")



# === ideal ===
min_age = min(df_means["Real_Age"].min(), df_means["Predicted_Age"].min()) - 5
max_age = max(df_means["Real_Age"].max(), df_means["Predicted_Age"].max()) + 5
plt.plot([min_age, max_age], [min_age, max_age], color='red', linestyle='--', label="Ideal (y = x)")
...
plt.ylim(min_age, max_age)
plt.xlabel("Real Age")
plt.ylabel("Predicted Age (Mean across Repeats)")
plt.title("Predicted vs Real Age (Mean Prediction per Subject)")
plt.legend()
...
# === Metrics box (no std) ===
textstr = (
    f"MAE:  {mae_mean_subj:.2f}\n"
    f"RMSE: {rmse_mean_subj:.2f}\n"
    f"R²:   {r2_mean_subj:.2f}"
)
plt.text(0.95, 0.05, textstr, transform=plt.gca().transAxes,
         fontsize=12, verticalalignment='bottom', horizontalalignment='right',
...
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "ADRC_bimodal_ScatterPlot_meanPerSubject.png"), dpi=300)
plt.close()




# === SCATTER PLOT: mean prediction per subject with trend line ===
import os
import numpy as np
import matplotlib.pyplot as plt
from sklearn.linear_model import LinearRegression
...
# --- Aggregate mean prediction per subject ---
df_means = df_preds_bimodal.groupby("Subject_ID").agg({
    "Real_Age": "first",
    "Predicted_Age": "mean"
}).reset_index()

# --- Metrics on df_means (no std) ---
mae_mean_subj  = mean_absolute_error(df_means["Real_Age"], df_means["Predicted_Age"])
rmse_mean_subj = mean_squared_error(df_means["Real_Age"], df_means["Predicted_Age"], squared=False)
r2_mean_subj   = r2_score(df_means["Real_Age"], df_means["Predicted_Age"])
...
plt.figure(figsize=(8, 6))
plt.scatter(
    df_means["Real_Age"], df_means["Predicted_Age"],
    s=40, color="steelblue", alpha=1.0,
    label="Mean Prediction"
)

# --- Linear regression fit ---
reg_mean = LinearRegression().fit(
    df_means["Real_Age"].values.reshape(-1, 1),
    df_means["Predicted_Age"].values
)
slope_m, intercept_m = reg_mean.coef_[0], reg_mean.intercept_

# Line spanning full axis range
min_age = df_means["Real_Age"].min() - 5
max_age = df_means["Real_Age"].max() + 5
x_line  = np.array([min_age, max_age]).reshape(-1, 1)
y_line  = reg_mean.predict(x_line)

# --- Trend line with equation in legend ---
eq_label = fr"$y = {slope_m:.2f}x + {intercept_m:.2f}$"
plt.plot(x_line, y_line,
         color="navy", lw=2, alpha=0.5,
         label=eq_label)
...
plt.text(0.95, 0.05, metrics_str, transform=plt.gca().transAxes,
...
plt.xlabel("Real Age")
plt.ylabel("Predicted Age (Mean across Repeats)")
plt.title("Predicted vs Real Age – Mean per Subject (Bimodal ADRC)")
plt.legend()
plt.grid(True)
plt.tight_layout()

# --- Save figure ---
plt.savefig(os.path.join(output_dir, "scatter_real_vs_pred_mean_per_subject.png"), dpi=300)
plt.close()
...
plt.savefig(os.path.join(output_dir, "learning_curve_final_model.png"), dpi=300)
plt.close()
