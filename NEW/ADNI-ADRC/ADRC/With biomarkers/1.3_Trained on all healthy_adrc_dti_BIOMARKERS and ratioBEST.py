...
df_healthy_adrc_dti["AB40"] = pd.to_numeric(df_healthy_adrc_dti["AB40"], errors='coerce')
df_healthy_adrc_dti["AB42"] = pd.to_numeric(df_healthy_adrc_dti["AB42"], errors='coerce')

# Step 2: Compute the ratio safely
df_healthy_adrc_dti["AB_ratio"] = df_healthy_adrc_dti["AB42"] / df_healthy_adrc_dti["AB40"]
df_healthy_adrc_dti["AB_ratio"].replace([np.inf, -np.inf], np.nan, inplace=True)
...
df_healthy_adrc_dti[biomarker_cols] = df_healthy_adrc_dti[biomarker_cols].apply(
    lambda col: (col - col.mean()) / (col.std() + 1e-8)
)

# Fill NaNs with -10 (clear out-of-distribution marker)
df_healthy_adrc_dti[biomarker_cols] = df_healthy_adrc_dti[biomarker_cols].fillna(-10)
...
for subject, matrix_log in log_thresholded_connectomes_adrc_dti_healthy.items():
    try:
        ...
        global_feat = torch.cat([demo_tensor, graph_tensor, biomarker_tensor], dim=0)  # [12]
...
class BrainAgeGATv2_ADNI(nn.Module):
    def __init__(self):
        ...
        # Fusion will now be: 16 + 32 + 32 
        self.fc = nn.Sequential(
            nn.Linear(128 + 16+32+32, 128),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(128, 64),
            nn.ReLU(),
    ...
    def forward(self, data):
        ...
        graph_embed = self.graph_head(global_feats[:, 2:6])  # 4 graph metrics

        bio_embed = self.bio_head(global_feats[:, 6:13])     # 6 biomarkers

        
        global_embed = torch.cat([meta_embed, graph_embed, bio_embed], dim=1)  # [16 + 32 + 32 = 80]
...
# Compute overall MAE and R²
mae = mean_absolute_error(df_preds["Real_Age"], df_preds["Predicted_Age"])
r2 = r2_score(df_preds["Real_Age"], df_preds["Predicted_Age"])
...
#mean from folds and repeats espochs after early stopping~70
epochs = 90
