#FULL FINE TUNING
#Full fine tuniing adni-adrc dti fmri
...
# === LOAD MODEL + FULL PRETRAINED WEIGHTS ===
model = DualGATv2_EarlyFusion().to(device)

# Load pretrained weights from ADNI (bimodal)
pretrained_weights = torch.load("/home/bas/Desktop/Paula DTI_fMRI Codes/ADNI/brainage_adni_bimodal_pretrained.pt", map_location=device)

# Load all weights directly (no layer exclusion needed)
model.load_state_dict(pretrained_weights)

print(f"  >> Pretrained weights loaded from bimodal_adni_pretrained.pt (fully matched).")
...
# === STRATIFICATION SETUP (based on age bins) ===
...
for fold, (train_idx, test_idx) in enumerate(skf.split(graph_data_list_adrc_bimodal, age_bins)):
    ...
    for repeat in range(repeats_per_fold):
        ...
        # === MODEL + LOAD PRETRAINED WEIGHTS FROM ADNI ===
        model = DualGATv2_EarlyFusion().to(device)
        pretrained_weights = torch.load("/home/bas/Desktop/Paula DTI_fMRI Codes/ADNI/brainage_adni_bimodal_pretrained.pt", map_location=device)
        model.load_state_dict(pretrained_weights)  # Full match expected
        ...
        for epoch in range(epochs):
            ...
            if test_loss < best_loss:
                best_loss = test_loss
                patience_counter = 0
                torch.save(model.state_dict(), f"bimodal_finetuned_fold_{fold+1}_rep_{repeat+1}.pt")
            else:
                patience_counter += 1
...
for fold, (train_idx, test_idx) in enumerate(skf_bimodal.split(graph_data_list_adrc_bimodal, age_bins_bimodal)):
    ...
    for rep in range(repeats_per_fold):
        ...
        model = DualGATv2_EarlyFusion().to(device)
        model.load_state_dict(torch.load(f"bimodal_finetuned_fold_{fold+1}_rep_{rep+1}.pt"))
...
plt.title("Predicted vs Real Ages Full fine tuned ADNI - ADRC, Bimodal Model: DTI - FMRI")
plt.legend()
