...
# ==============================================================
# SHAP para EDGES DTI – modelo BrainAgeGATv2 (AD-DECODE)
# ==============================================================

import shap, os, torch
import pandas as pd
from torch_geometric.data import Data

# ── Asegúrate de tener el modelo en eval y en el dispositivo correcto ──
model.eval()
model = model.to(device)

# ----------------------------------------------------------------------
# 1) Wrapper: solo varia edge_attr de DTI; todo lo demás permanece fijo
# ----------------------------------------------------------------------
class EdgeSHAPWrapperAD(torch.nn.Module):
    """
    Receives a batch of edge-attribute tensors and returns model prediction
    after replacing edge_attr in a cloned Data object. Global_feats, node_feats
    and topology remain unchanged – we isolate the contribution of each edge.
    """
    def __init__(self, model: torch.nn.Module, base_data: Data):
        super().__init__()
        self.model = model
        # Clone once; we will replace edge_attr each call
        self.base_data = base_data.clone().to(next(model.parameters()).device)

        # Ensure .batch exists (needed for global_mean_pool)
        if not hasattr(self.base_data, "batch"):
    ...
    def forward(self, edge_attr_batch):
        outputs = []
        for ea in edge_attr_batch:            # ea shape → [num_edges]
            d = self.base_data.clone()
            d.edge_attr = ea
            out = self.model(d).squeeze(0)    # scalar age prediction
            outputs.append(out)
        return torch.stack(outputs)           # shape → [B]

# ----------------------------------------------------------------------
# 2) Loop through all AD-DECODE graphs and compute SHAP per subject
# ----------------------------------------------------------------------
os.makedirs("shap_outputs_addecode", exist_ok=True)

for idx, data in enumerate(graph_data_list_addecode, 1):
    sid = data.subject_id
    print(f"[{idx}/{len(graph_data_list_addecode)}]  Subject {sid}")

    # Move single-subject graph to device & keep a clone as baseline
    base_data = data.to(device)

    # --------- Wrapper (edges only) ---------
    wrapper = EdgeSHAPWrapperAD(model, base_data)

    # --------- Prepare baseline + actual input ---------
    baseline = torch.zeros_like(base_data.edge_attr).unsqueeze(0)   # [1, num_edges]
    input_ea = base_data.edge_attr.unsqueeze(0)                     # [1, num_edges]

    # --------- Gradient SHAP ---------
    explainer  = shap.GradientExplainer(wrapper, baseline)
    shap_vals  = explainer.shap_values(input_ea)       # returns list[0]
    shap_edge = np.squeeze(shap_vals[0])  # [num_edges]

    # --------- Edge indices ---------
    edges = base_data.edge_index.cpu().numpy().T       # [num_edges, 2]
