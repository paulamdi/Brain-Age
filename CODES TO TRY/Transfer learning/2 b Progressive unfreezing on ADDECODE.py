...
pretrained_path = "brainage_adni1_pretrained.pt"
...
print(" Loaded full pretrained weights from ADNI.")

# Freeze all layers
for param in model.parameters():
    param.requires_grad = False

# Unfreeze only the MLP head initially
for param in model.fc.parameters():
    param.requires_grad = True


# Train global features from the beggining 
for param in model.meta_head.parameters():
    param.requires_grad = True

for param in model.graph_head.parameters():
    param.requires_grad = True

for param in model.pca_head.parameters():
    param.requires_grad = True
...
for fold, (train_idx, test_idx) in enumerate(skf.split(graph_data_list_addecode, age_bins)):
    ...
    for repeat in range(repeats_per_fold):
        ...
        model = BrainAgeGATv2(global_feat_dim=16).to(device)  # Create model instance for AD-DECODE
        pretrained_weights = torch.load("brainage_adni1_pretrained.pt", map_location=device)

        
        model.load_state_dict(pretrained_weights)  # Load only matching weights

        

        # === INITIAL FREEZE ===
        for name, param in model.named_parameters():
            if any(layer in name for layer in ["gnn1", "gnn2", "gnn3", "gnn4", "node_embed"]):
                param.requires_grad = False  # Freeze GNN layers
            else:
                param.requires_grad = True   # Unfreeze global MLP and metadata heads
        ...
        for epoch in range(epochs):
            ...
            if epoch == 10:
                print(" >> Unfreezing gnn4")
                for name, param in model.named_parameters():
                    if "gnn4" in name:
                        param.requires_grad = True
        
            elif epoch == 20:
                print(" >> Unfreezing gnn3")
                for name, param in model.named_parameters():
                    if "gnn3" in name:
                        param.requires_grad = True
        
            elif epoch == 30:
                print(" >> Unfreezing gnn2")
                for name, param in model.named_parameters():
                    if "gnn2" in name:
                        param.requires_grad = True
        
            elif epoch == 40:
                print(" >> Unfreezing gnn1 and node_embed")
                for name, param in model.named_parameters():
                    if "gnn1" in name or "node_embed" in name:
                        param.requires_grad = True
            ...
            if test_loss < best_loss:
                best_loss = test_loss
                patience_counter = 0
                torch.save(model.state_dict(), f"finetuned_from_adni1_fold_{fold+1}_rep_{repeat+1}.pt")
            else:
                patience_counter += 1
...
for fold, (train_idx, test_idx) in enumerate(skf.split(graph_data_list_addecode, age_bins)):
    ...
    for rep in range(repeats_per_fold):
        ...
        model.load_state_dict(torch.load(f"finetuned_from_adni1_fold_{fold+1}_rep_{rep+1}.pt"))  # Load correct model
        model.eval()
...
# ---------------------------  CONFIG  ---------------------------
OUT_DIR = "adni_addecode_FINETUNED1_training_eval_plots_save"        # where CSV + figs go
...
for fold, (train_idx, test_idx) in enumerate(skf_addecode.split(
    ...
    for rep in range(REPEATS_PER_FOLD):
        ...
        # ----- Load trained model -----
        model = BrainAgeGATv2(global_feat_dim=16).to(device)
        ckpt_path = f"finetuned_from_adni1_fold_{fold+1}_rep_{rep+1}.pt"
...
csv_path = os.path.join(OUT_DIR, "cv_predictions_adni_addecode_FINETUNED1.csv")
...
plt.title("Predicted vs Real Ages adni-addecode FINETUNED1 (All Repeats)")
