# === Clustering SHAP Contrastive Embeddings from Full  fine tuned model===
...
df_embed = pd.read_csv("FULL_FINE_TUNING_shap_embeddings.csv")  # Must include 'Subject_ID' + dim_0 ... dim_n
...
plt.title(f"UMAP of Full fine tuned nSHAP Embeddings with K-Means Clusters (k={k})")
plt.legend(title="Cluster", bbox_to_anchor=(1.05, 1), loc='upper left')
plt.tight_layout()
plt.show()

# === 6. (Optional) Save cluster assignments ===
df_embed[["Subject_ID", "UMAP1", "UMAP2","Cluster"]].to_csv("FULL_FINE_TUNING_shap_embedding_clusters.csv", index=False)
