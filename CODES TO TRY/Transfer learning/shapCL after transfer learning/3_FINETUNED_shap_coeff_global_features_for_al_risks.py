# ADDECODE 
    ...
    # SHAP COEFFICIENTS AFTER FULL FINE TUNED MODEL ON ALL RISKS SUBJECTS
...
# WE KEEP ALL RISKS TO CALCULATE SHAP COEFFICIENTS FOR ALL
...
model.load_state_dict(torch.load("FULL_FINE_TUNED_model_trained_on_all_healthy.pt"))   #### OR FULL PATH
model.eval()
...
df_shap.to_csv("FULL_FINE_TUNING_shap_global_features_all_subjects.csv", index=False)
print("Saved: FULL_FINE_TUNING_shap_global_features_all_subjects.csv")
...
df = pd.read_csv("FULL_FINE_TUNING_shap_global_features_all_subjects.csv")

# Drop the 'Age' column
df_no_age = df.drop(columns=["Age"])

# Save the new CSV without Age
df_no_age.to_csv("FULL_FINE_TUNING_shap_global_features_no_age.csv", index=False)
