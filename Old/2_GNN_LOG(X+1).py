...
log_transformed_connectomes = {}

# Iterate on matched_connectomes
for subject, matrix in matched_connectomes.items():
    #  log(x + 1) to avoid log(0)
    matrix_log = np.log1p(matrix)  # Log(x + 1)
    
    # Almacenar las matrices log-transformadas en un nuevo diccionario
    log_transformed_connectomes[subject] = matrix_log

# Verificar que se han transformado las matrices correctamente
# Mostrar las primeras 5 matrices log-transformadas
for subject, matrix_log in list(log_transformed_connectomes.items())[:5]:
...
for subject, matrix_log in log_transformed_connectomes.items():
