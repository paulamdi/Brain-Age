...
#####################  MASK SPECIFIC REGIONS IN CONNECTOME MATRIX  #####################

def mask_regions(matrix, regions_to_remove):
    """
    Sets to zero all rows and columns of specified brain regions in the connectome matrix.

    Parameters:
        matrix (pd.DataFrame): Original 84x84 connectome matrix.
        regions_to_remove (list): List of region indices to mask (0-based).

    Returns:
        masked_matrix (pd.DataFrame): Modified matrix with masked regions.
    """
    matrix = matrix.copy()
...
#####################  APPLY REGION MASK TO CONNECTOMES  #####################

# === Indices of first 10 regions to ablate ===
regions_to_remove = list(range(16))

# === Create new dictionary with masked matrices ===
masked_log_connectomes = {}

for subject, matrix in log_thresholded_connectomes.items():
    masked_matrix = mask_regions(matrix, regions_to_remove)
    masked_log_connectomes[subject] = masked_matrix
...
#####################  CONVERT MASKED MATRICES TO GRAPHS #####################

masked_graph_data_list = []

for subject, matrix in masked_log_connectomes.items():
    edge_index, edge_attr, node_features = matrix_to_graph(matrix)

    age_row = df_metadata.loc[df_metadata["DWI_fixed"] == subject, "age"]
    if not age_row.empty:
        age = torch.tensor([age_row.values[0]], dtype=torch.float)
        data = Data(x=node_features, edge_index=edge_index, edge_attr=edge_attr, y=age)
        masked_graph_data_list.append(data)

# Optional: print to verify
print(f"Total masked graphs: {len(masked_graph_data_list)}")
...
masked_train_losses_all = []
masked_test_losses_all = []
...
for fold, (train_idx, test_idx) in enumerate(kf.split(graph_data_list)):
    ...
    # Train and test data
    train_data = [masked_graph_data_list[i] for i in train_idx]
    test_data = [masked_graph_data_list[i] for i in test_idx]
    ...
    # Save best model
    best_model_path = f"masked_model_fold_{fold+1}.pt"
    best_loss = float('inf')
    patience_counter = 0
    ...
    for epoch in range(epochs):
        ...
        masked_train_losses.append(train_loss)
        masked_test_losses.append(test_loss)
    ...
    masked_train_losses_all.append(masked_train_losses)
    masked_test_losses_all.append(masked_test_losses)
...
for fold in range(k):
    plt.plot(masked_train_losses_all[fold], label=f'Masked Train - Fold {fold+1}', linestyle='dashed')
    plt.plot(masked_test_losses_all[fold], label=f'Masked Test - Fold {fold+1}')

    
    plt.title("Learning curve (masked)")
...
for fold, (train_idx, test_idx) in enumerate(kf.split(graph_data_list)):
    ...
    # Upload model from training
    model = BrainAgeGNN()
    model.load_state_dict(torch.load(f"masked_model_fold_{fold+1}.pt"))
    ...
    with torch.no_grad():
        for data in test_loader:
            output = model(data).view(-1)  # Predicciones d
            masked_y_pred.extend(output.tolist())
            masked_y_true.extend(data.y.tolist())




# Lists to arrays
masked_y_true = np.array(masked_y_true)
masked_y_pred = np.array(masked_y_pred)


# === Calculate MAE and  R² ===
masked_mae = mean_absolute_error(masked_y_true, masked_y_pred)
masked_r2 = r2_score(masked_y_true, masked_y_pred)
...
# === Graph real vd predicted values===
plt.figure(figsize=(8, 6))
plt.scatter(masked_y_true, masked_y_pred, alpha=0.7, edgecolors='k', label="Predictions")
plt.plot([min(y_true), max(y_true)], [min(y_true), max(y_true)], color="red", linestyle="dashed", label="Ideal (y=x)")


textstr = f"MAE: {masked_mae:.2f}\nR²: {masked_r2:.2f}"
plt.text(0.95, 0.05, textstr, transform=plt.gca().transAxes,
         fontsize=12, verticalalignment='bottom', horizontalalignment='right',
         bbox=dict(boxstyle="round,pad=0.3", edgecolor="black", facecolor="lightgray"))
...
plt.xlabel("Real Age")
plt.ylabel("Predicted Age")
plt.title("Real vs Predicted Ages (Masked Model)")
plt.legend(loc="upper left")  
plt.grid(True)
plt.show()


############### PREDICT SOME AGES — MASKED MODEL #################

import random

# Select 5 random examples from masked predictions
random_indices = random.sample(range(len(masked_y_true)), 5)

print("\n Example predictions (Masked Model):")
for idx in random_indices:
    print(f"Patient {idx+1}: Real Age = {masked_y_true[idx]:.1f} years | Prediction = {masked_y_pred[idx]:.1f} years")
...
#####################  ABLATION-BASED EDGE IMPORTANCE FUNCTION  #####################

def compute_edge_importance_matrix(model, data, matrix_size=84):
    """
    For a given graph and trained model:
    - Iteratively removes each edge (sets its weight to 0)
    - Measures how much the model's age prediction changes
    - Stores the change in a symmetric 84x84 importance matrix

    Returns:
        importance_matrix (numpy array): Symmetric matrix with edge importance values
    """

    import torch, copy

    # === 1. Get the original prediction from the model ===
    with torch.no_grad():
        original_pred = model(data).item()

    # === 2. Clone edge list and weights to manipulate them ===
    edge_index = data.edge_index.clone()
    edge_attr = data.edge_attr.clone()

    # === 3. Initialize the 84x84 importance matrix ===
    importance_matrix = np.zeros((matrix_size, matrix_size))

    # === 4. Iterate through all edges in the graph ===
    for i in range(edge_index.size(1)):
        i_node = edge_index[0, i].item()
        j_node = edge_index[1, i].item()

        # Skip lower triangle (we only process each undirected edge once)
        if i_node >= j_node:
            continue

        # === 5. Copy the graph and zero-out the selected edge ===
        new_data = copy.deepcopy(data)
        new_data.edge_attr[i] = 0.0  # remove the edge by setting its weight to 0

        # === 6. Get new prediction after edge is removed ===
        with torch.no_grad():
            new_pred = model(new_data).item()

        # === 7. Calculate the prediction difference ===
        importance = abs(original_pred - new_pred)

        # === 8. Store the importance score symmetrically in the matrix ===
        importance_matrix[i_node, j_node] = importance
        importance_matrix[j_node, i_node] = importance

    return importance_matrix

#####################  MULTI-PATIENT ANALYSIS  #####################

# === 1. Load region names (index 0 to 83) ===
...
# === 2. Choose a fold and select some test patients ===
example_fold = 0
_, test_idx = list(kf.split(graph_data_list))[example_fold]
selected_patients = list(range(len(graph_data_list)))  # ← Usa todos los pacientes disponibles

# ????????????????????? use all patients???????????????????????????  



# === 3. Load the trained GNN model for that fold ===
model = BrainAgeGNN()
model.load_state_dict(torch.load(f"model_fold_{example_fold+1}.pt"))
model.eval()  # Important: set to evaluation mode


# === 4. Analyze each patient and print their top 5 most important connections ===
for idx in selected_patients:
    data = graph_data_list[idx]  # Get the graph for this patient

    # === Compute the edge importance matrix using ablation ===
    importance_matrix = compute_edge_importance_matrix(model, data)

    # === Extract top 5 most important edges (only upper triangle) ===
    flat = importance_matrix[np.triu_indices(84, k=1)]  # Flatten upper triangle
    top_indices = np.argpartition(-flat, 5)[:5]  # Indices of top 5 deltas (fast method)
    
    i_indices, j_indices = np.triu_indices(84, k=1)
    top_connections = [(i_indices[k], j_indices[k], flat[k]) for k in top_indices]
    
    # Sort top connections by importance (descending)
    top_connections.sort(key=lambda x: -x[2])
    
    # === Get patient metadata ===
    dwi_id = df_matched_connectomes.iloc[idx]["DWI_fixed"]
    age = df_matched_connectomes.iloc[idx]["age"]
    
    print(f"\nPatient index: {idx} — DWI ID: {dwi_id} — Age: {age:.1f} — Top 5 influential connections:")
    
    for i, j, delta in top_connections:
        region_pair = f"{region_names[i]} ↔ {region_names[j]}"
        print(f"{region_pair:55s} | Δ prediction: {delta:.2f}")


 
##################### SAVE TOP CONNECTIONS TO CSV #####################

import csv

# Path where the CSV will be saved
output_path = "top_connections_by_patient.csv"

# Open CSV file and write header
with open(output_path, mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(["Patient_Index", "DWI_ID", "Age", "Region_1", "Region_2", "Delta"])

    # Re-run the loop and write results
    for idx in selected_patients:
        data = graph_data_list[idx]
        importance_matrix = compute_edge_importance_matrix(model, data)

        flat = importance_matrix[np.triu_indices(84, k=1)]
        ...
        for i, j, delta in top_connections:
            ...
            writer.writerow([idx, dwi_id, age, region1, region2, round(delta, 4)])



################  MASKED ##

# === 2. Choose a fold and select all patients from masked data ===
example_fold = 0
_, test_idx = list(kf.split(masked_graph_data_list))[example_fold]
selected_patients = list(range(len(masked_graph_data_list)))  # Use all masked patients



# === Load masked model ===
model = BrainAgeGNN()
model.load_state_dict(torch.load(f"masked_model_fold_{example_fold+1}.pt"))
model.eval()

selected_patients = list(range(len(masked_graph_data_list)))  # TODOS los pacientes con conexiones eliminadas

# === CSV file for masked model ===
output_path = "top_connections_by_patient_MASKED.csv"
...
with open(output_path, mode='w', newline='') as file:
    ...
    for idx in selected_patients:
        data = masked_graph_data_list[idx]  # ← usamos la versión masked
