...
############ DEMOGRAFIC FEATURES (SEX AND APOE) ########################
...
# === Create a single genotype string column from APOE A1 + A2 ===
df_matched_adni_healthy["genotype"] = (
    df_matched_adni_healthy["APOE A1"].astype(str) + "_" + df_matched_adni_healthy["APOE A2"].astype(str)
)

# === Define demographic features to use ===
categorical_label_cols = ["Sex"]
categorical_ordered_cols = ["genotype"]

# Drop rows with missing values in selected features
required_cols = categorical_label_cols + categorical_ordered_cols
df_matched_adni_healthy = df_matched_adni_healthy.dropna(subset=required_cols).reset_index(drop=True)

# === Label encode 'Sex' ===
for col in categorical_label_cols:
    le = LabelEncoder()
    df_matched_adni_healthy[col] = le.fit_transform(df_matched_adni_healthy[col].astype(str))

# === Label encode APOE genotype ===
for col in categorical_ordered_cols:
    le = LabelEncoder()
    df_matched_adni_healthy[col] = le.fit_transform(df_matched_adni_healthy[col].astype(str))

# === Final metadata DataFrame with encoded values ===
meta_df_adni = df_matched_adni_healthy[categorical_label_cols + categorical_ordered_cols].astype(float)
