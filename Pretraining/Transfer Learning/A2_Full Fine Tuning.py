...
#This approach enables the model to adapt more fully to the new data. 
...
for fold, (train_idx, test_idx) in enumerate(skf.split(graph_data_list_addecode, age_bins)):
    ...
    for repeat in range(repeats_per_fold):
        ...
        model = BrainAgeGATv2(global_feat_dim=8).to(device)  # Create model instance for AD-DECODE
        pretrained_weights = torch.load("brainage_adni_pretrained.pt", map_location=device)
        ...
        for epoch in range(epochs):
            ...
            if test_loss < best_loss:
                best_loss = test_loss
                patience_counter = 0
                torch.save(model.state_dict(), f"finetuned_fold_{fold+1}_rep_{repeat+1}.pt")
            else:
                patience_counter += 1
...
for fold, (train_idx, test_idx) in enumerate(skf.split(graph_data_list_addecode, age_bins)):
    ...
    for rep in range(repeats_per_fold):
        ...
        model = BrainAgeGATv2(global_feat_dim=8).to(device)
        model.load_state_dict(torch.load(f"finetuned_fold_{fold+1}_rep_{rep+1}.pt"))
        model.eval()
