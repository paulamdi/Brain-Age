# Transfer Learning via Pretraining and Progressive Fine-tuning
# FEATURES AND NODE FEATURES  the same in adni and addecode
#Node features: FA, VOL​
#Features : Sex, APOE​


#transfer learning by pretraining a GATv2 model on ADNI healthy subjects and 
#fine-tuning it on the AD-DECODE dataset using progressive unfreezing.
...
all_required_cols = categorical_label_cols + categorical_ordered_cols
...
meta_df = addecode_healthy_metadata[categorical_label_cols + categorical_ordered_cols]
...
# === Node-wise normalization across subjects ===
def normalize_multimodal_nodewise(feature_dict):
    all_features = torch.stack(list(feature_dict.values()))  # [N_subjects, 84, 2]
...
# Visual check of first 3 transformed matrices
...
print(f"Total connectomes after filtering: {len(matched_connectomes_healthy_addecode)}")
print(f"Total node features available: {len(normalized_node_features_dict)}")
print(f"Total metadata available: {len(subject_to_meta_addecode)}")
...
for subject, matrix_log in log_thresholded_connectomes.items():
    ...
    if subject not in normalized_node_features_dict:
        continue  # Skip if no node features
...
# ==== BLOCK 5: Training and Evaluation Functions ====
...
def train(model, train_loader, optimizer, criterion):
    ...
    for data in train_loader:
        ...
        output = model(data).view(-1)  # Predict
...
# ==== BLOCK 6: Cross-validation Setup + Progressive Unfreezing ====
...
# Prepare age bins for stratified split
...
for fold, (train_idx, test_idx) in enumerate(skf.split(graph_data_list_addecode, age_bins)):
    ...
    for repeat in range(repeats_per_fold):
        ...
        seed_everything(42 + repeat)  # Ensure reproducibility for each repeat
        ...
        # === Load pretrained ADNI weights  ===
        pretrained_weights = torch.load("brainage_adni_pretrained.pt", map_location=device)
        model.load_state_dict(pretrained_weights)

        # Freeze all layers except final FC layers
        for name, param in model.named_parameters():
            if "fc" not in name:
                param.requires_grad = False
        print("Initial freezing: only FC layers are trainable.")
        ...
        for epoch in range(epochs):
            ...
            # At the start, freeze GNN layers 2–4
            if epoch == 0:
                for name, param in model.named_parameters():
                    if any(layer in name for layer in ["gnn2", "gnn3", "gnn4"]):
                        param.requires_grad = False  # freeze these layers
        
            # After 10 epochs, unfreeze gnn2
            elif epoch == 10:
                for name, param in model.named_parameters():
                    if "gnn2" in name:
                        param.requires_grad = True  # allow gnn2 to be trained
        
            # After 20 epochs, unfreeze gnn3
            elif epoch == 20:
                for name, param in model.named_parameters():
                    if "gnn3" in name:
                        param.requires_grad = True  # allow gnn3 to be trained
        
            # After 30 epochs, unfreeze gnn4
            elif epoch == 30:
                for name, param in model.named_parameters():
                    if "gnn4" in name:
                        param.requires_grad = True  # allow gnn4 to be trained
            ...
            if test_loss < best_loss:
                best_loss = test_loss
                patience_counter = 0
                torch.save(model.state_dict(), f"progressive_unfreezing2_fold_{fold+1}_rep_{repeat+1}.pt")  # save best model
            else:
                patience_counter += 1
...
plt.title("Learning Curves (All Repeats) Progressive Unfreezing Same Features for ADNI and ADDECODE")
...
plt.xlabel("Epoch")
plt.ylabel("Smooth L1 Loss")
plt.title("Learning Curve (Mean ± Std Across All Folds/Repeats) Progressive Unfreezing Same Features for ADNI and ADDECODE")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
# === Evaluate each fold and repetition ===
for fold, (train_idx, test_idx) in enumerate(skf.split(graph_data_list_addecode, age_bins)):
    ...
    for rep in range(repeats_per_fold):
        ...
        # === Load the best model saved for this fold and repeat ===
        model = BrainAgeGATv2(global_feat_dim=6).to(device)
        model.load_state_dict(torch.load(f"progressive_unfreezing2_fold_{fold+1}_rep_{rep+1}.pt"))
        model.eval()
...
plt.title("Predicted vs Real Age: Progressive unfreezing Same Features for ADNI and ADDECODE")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
