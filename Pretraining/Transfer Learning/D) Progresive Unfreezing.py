...
for fold, (train_idx, test_idx) in enumerate(skf.split(graph_data_list_addecode, age_bins)):
    ...
    for repeat in range(repeats_per_fold):
        ...
        # === Load pretrained ADNI weights (partial loading) ===
        pretrained_weights = torch.load("brainage_adni_pretrained.pt", map_location=device)
        excluded_layers = ["node_embed", "fc.0"]
        filtered_weights = {
            k: v for k, v in pretrained_weights.items()
        ...
        # === Freeze all GNN layers initially ===
        for name, param in model.named_parameters():
            if any(layer in name for layer in ["gnn1", "gnn2", "gnn3", "gnn4", "bn1", "bn2", "bn3", "bn4"]):
        ...
        for epoch in range(epochs):
            ...
            if test_loss < best_loss:
                best_loss = test_loss
                patience_counter = 0
                torch.save(model.state_dict(), f"model_fold_{fold+1}_rep_{repeat+1}.pt")  # save best model
            else:
                patience_counter += 1
...
plt.title("Learning Curves (All Repeats) Freezing first layer")
...
plt.xlabel("Epoch")
plt.ylabel("Smooth L1 Loss")
plt.title("Learning Curve (Mean ± Std Across All Folds/Repeats) Freezing first layer")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
plt.title("Predicted vs Real Age: Freezing first layer")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
